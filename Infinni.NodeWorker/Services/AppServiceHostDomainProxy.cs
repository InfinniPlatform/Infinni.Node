...
{
    public class AppServiceHostDomainProxy : IAppServiceHost, IDisposable
    {
        public AppServiceHostDomainProxy(AppServiceOptions options)
        {
            if (options == null)
            {
        ...
        private static AppDomain CreateWorkerServiceDomain(AppServiceOptions options)
        {
        ...
        private static string GetDomainFriendlyName(string packageId, string packageVersion, string packageInstance)
        {
            return string.IsNullOrWhiteSpace(packageInstance)
                ...
                : $"{packageId}.{packageVersion}@{packageInstance}";
        }

        private static string GetDomainConfigurationFile(AppServiceOptions options, string domainApplicationBase)
        {
        ...
        private static void SetWorkerServiceHost(AppDomain domain, string serviceHostContractName, string serviceHostSearchPattern)
        {
            ...
            domain.DoCallBack(() =>
            {
                ...
                var worker = new Lazy<IAppServiceHost>(() => new AppServiceHostImplementation(contractName, searchPattern));
                SetDomainData(AppDomain.CurrentDomain, "Instance", worker);
            });
        }

        private static IAppServiceHost GetWorkerServiceHost()
        {
            var domain = AppDomain.CurrentDomain;
            var worker = GetDomainData<Lazy<IAppServiceHost>>(domain, "Instance");
            return worker.Value;
        }
        ...
        {
            return $"IWorkerServiceHost.{name}";
        }


        class DomainAssemblyResolver
        {
