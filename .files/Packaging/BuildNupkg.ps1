...
# Restore packages
& "$nugetPath" restore 'Infinni.Node.sln' -NonInteractive

# Build the solution
& "${Env:ProgramFiles(x86)}\MSBuild\14.0\bin\msbuild.exe" 'Infinni.Node.sln' /t:Clean /p:Configuration=$buildMode /verbosity:quiet /consoleloggerparameters:ErrorsOnly
& "${Env:ProgramFiles(x86)}\MSBuild\14.0\bin\msbuild.exe" 'Infinni.Node.sln' /p:Configuration=$buildMode /verbosity:quiet /consoleloggerparameters:ErrorsOnly
